...
A personal scheduling and task-prioritization app. Multi-user and offline-first.

- **Backend**: Rust, [`axum`](https://crates.io/crates/axum) 0.8,
  [`sqlx`](https://crates.io/crates/sqlx) 0.9 (SQLite). No domain logic: auth,
  row sync (snapshot + per-row last-write-wins), and static serving. Serves the
  API under `/api` and the built frontend at `/` with SPA fallback.
- **Frontend**: [Preact](https://preactjs.com/) 10 + TSX, bundled with
  [Vite](https://vite.dev/), with [`@preact/signals`](https://github.com/preactjs/signals),
  `preact-iso` routing, and `vite-plugin-pwa`. Offline-first: IndexedDB cache,
  optimistic ops, and background sync.
- **Types**: defined once in Rust (`backend/src/types`) and exported to TypeScript
  via [`ts-rs`](https://crates.io/crates/ts-rs) (regenerated by `cargo test`).
- **Auth**: stateless signed cookies (`axum-extra`), Argon2 password hashes.

```
backend/             Rust crate (lib + binaries: schedule, adduser).
backend/src/types/   Wire/row types; derive serde + ts_rs::TS (source of bindings).
backend/migrations/  SQLx migrations (embedded into the binary at build time).
frontend/            Vite app: index.html, src/, public/ (PWA icons).
frontend/src/lib/    Pure, framework-free domain logic (layout, ordering, ...).
frontend/src/data/   IndexedDB, API client, sync engine.
frontend/src/state/  Preact signals + operation-building mutations.
frontend/src/ui/     Feature folders + components (CSS Modules).
frontend/src/bindings/  Generated from backend types by cargo test (gitignored).
...
```bash
cargo test                 # Rust unit tests; also regenerates frontend/src/bindings
cd frontend && npm test    # Vitest (layout, ordering, ops, sync, parsing, ...)
```
...
| `FRONTEND_DIR` | `frontend/dist`                   | Directory served at `/` (the Vite build output).       |
